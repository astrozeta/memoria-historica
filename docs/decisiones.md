...
| 21 | **Ingesta vía endpoint admin protegido** (`POST /api/admin/ingest/<fuente>`) en lugar de scripts locales o GitHub Actions. | El ingestor necesita `NETLIFY_DATABASE_URL` que solo se inyecta en runtime de funciones de Netlify. Un endpoint protegido por `INGEST_SECRET` permite disparar la ingesta con `curl` desde cualquier sitio sin pasar credenciales de BD. A medio plazo se complementará con Scheduled Functions (cron) para actualización automática. |
